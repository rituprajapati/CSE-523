...
int Diff_Prolog::execute( const std::pair<int, int> &node, CnCContext &context ) const {
  ...
  // fprintf( stderr, "%s, %d, %d, %s \n", "--- Diff_Prolog -- Node: ", node.first, node.second, nodeInfo.toString().c_str());
...
int Diff_doIt::execute( const std::pair<int, int> &node, CnCContext &context ) const {
  ...
  if (left.s.length() != 0 && center.s.length() != 0 && right.s.length() != 0) {
    ...
    // fprintf( stderr, "%s, %d, %d, %s, %s, %s\n", "--- Diff_doIt ----- Node: ", node.first,
              // node.second,  left.toString().c_str(), center.toString().c_str(), right.toString().c_str());
...
struct gaxpy_sub_test : CnCContext {
  ...
  /* CnCContext Constructor
  /*----------------------------------------------------------------*/
   gaxpy_sub_test(int k, double thresh, int max_level, double (*funcA)(double), double (*funcB)(double))
   : 
     CnCContext( k, thresh, max_level),
     projectA_item(*this),
     projectB_item(*this), 
